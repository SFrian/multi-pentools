...
### **PHASE 2: Nmap Scan dengan NSE Secara Bergantian**
echo "### Starting PHASE 2: Nmap Scan ###"
while IFS= read -r target_url || [[ -n "$target_url" ]]; do
    target_url=$(echo "$target_url" | tr -d '\r')  # Bersihkan karakter tersembunyi
    TIMESTAMP=$(date '+%Y%m%d_%H%M%S')
    target_safe=$(echo "$target_url" | sed 's|https\?://||;s|/|_|g')

    # Daftar variabel NSE yang akan dijalankan
    nse_scripts=(
        "$nse_ftp"
        "$nse_tls"
        "$nse_ssl"
    ...
    for nse_script in "${nse_scripts[@]}"; do
        SCRIPT_NAME=$(echo "$nse_script" | awk '{print $1}' | sed 's/--script=//')
        NMAP_LOG="logs/nmap/${target_safe}_nmap_${SCRIPT_NAME}_$TIMESTAMP.txt"

        log_time "$NMAP_LOG" "### PHASE 2: Nmap Scan dengan $SCRIPT_NAME ###"
        log_time "$NMAP_LOG" "Starting AutoPent Nmap scan on $target_url dengan $SCRIPT_NAME..."

        nmap_output="-oN logs/nmap/${target_safe}_nmap_${SCRIPT_NAME}_$TIMESTAMP.txt"

        echo "Running: nmap $nse_script \"$target_url\" $nmap_output"  

        nmap $nse_script "$target_url" $nmap_output 2>&1 | tee -a "$NMAP_LOG"

        log_time "$NMAP_LOG" "Nmap Scan dengan $SCRIPT_NAME DONE ✅"
        echo "--------------------------------------------------------------------------"
    done
